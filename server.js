...
const sequelize = new Sequelize(
  process.env.DB_NAME || "sql10726853",
  process.env.DB_USER || "sql10726853",
  process.env.DB_PASSWORD || "kvTj46dr8M",
  {
...
app.get("/api/pro", async (req, res) => {
  ...
  try {
    ...
    if (categoria) {
      query += ` INNER JOIN subcategorias ON productos.id_subcategoria = subcategorias.id_subcategoria WHERE subcategorias.nombre_subcategoria = ?`;
  ...
  try {
    ...
    if (clienteEmail) {
      ...
      **Detalles de la compra:**
      ...
      - **Total a pagar:** ${total}
      - **Método de pago: Transfrencia bancaria
...
app.put("/api/productos/:id_producto", async (req, res) => {
  ...
  try {
    ...
    const result = await sequelize.query(
      `UPDATE productos
       SET codigo_producto = ?, descripcion = ?, precio_unitario = ?, stock = ?, id_subcategoria = ?
       WHERE id_producto = ?`,
      {
        replacements: [
          codigo_producto,
    ...
    if (result[1] > 0) {
      // `result[1]` contiene el número de filas afectadas
...
app.get("/api/clientes", async (req, res) => {
  ...
  try {
    ...
    console.error("Error al obtener los clientes:", error.message); // Más detalle del error
