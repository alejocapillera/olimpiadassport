<template>
  <div class="productos-container">
    ...
    >
      ...
      <b-button @click="showModalDetalles = false">Cerrar</b-button>
    </b-modal>
    ...
    >
      <form @submit.prevent="editarProducto">
        ...
        <b-form-group label="Subcategoría">
          <b-form-select
            v-model="productoActual.id_subcategoria"
...
export default {
  ...
  methods: {
    ...
    async fetchSubcategorias() {
      if (this.selectedCategoria) {
        try {
          ...
          this.subcategorias = response.data.map((subcategoria) => ({
            value: subcategoria.id_subcategoria,
            text: subcategoria.nombre_subcategoria,
          }));
        } catch (error) {
          console.error(
        ...
        this.subcategorias = []; // Limpia las subcategorías si no hay categoría seleccionada
      }
    },
    ...
    abrirModalEditar(producto) {
      this.productoActual = { ...producto };
      this.selectedCategoria = producto.id_subcategoria; // Asigna el ID de la subcategoría
      this.fetchSubcategorias(); // Carga las subcategorías
