  ...
  computed: {
    productosFiltrados() {
      ...
      return this.productos.filter((producto) => {
        const coincideCategoria = this.categoriaId
          ? producto.id_subcategoria === this.categoriaId
          : true;
        const coincideBusqueda = producto.descripcion
          .toLowerCase()
    ...
    async fetchProductos() {
      try {
        const response = await axios.get("http://localhost:3306/api/pro", {
          params: {
            categoriaId: this.categoriaId, // Pasar el id_categoria como parámetro
          },
        });
